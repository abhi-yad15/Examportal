        ...
        if($type=="round2"){  
          ...
          while($row5 = mysqli_fetch_array($result7,MYSQLI_ASSOC)){
              ...
              while($row = mysqli_fetch_array($result1,MYSQLI_ASSOC)){
                  ...
                  if($row1['skill1']!=0)
                    $skill1q++;
                  if($row1['skill1']!=0 && $row['score']>0)
                    $skill1a++;
       
                  if($row1['skill2']!=0)
                    $skill2q++;
                  if($row1['skill2']!=0 && $row['score']>0)
                    $skill2a++;
               
                  if($row1['skill3']!=0)
                    $skill3q++;
                  if($row1['skill3']!=0 && $row['score']>0)
                    $skill3a++;
               
                  if($row1['skill4']!=0)
                    $skill4q++;
                  if($row1['skill4']!=0 && $row['score']>0)
                    $skill4a++;
               
                  if($row1['skill5']!=0)
                    $skill5q++;
                  if($row1['skill5']!=0 && $row['score']>0)
                    $skill5a++;           
                }//echo $skill5q; 
              ?>
            ...
            ?>    <td><input type='button' id='report-submit' value='Section3' onClick="displayDiv('<?php echo $name ?>', '<?php echo $unattempted ?>','<?php echo $attempted ?>','<?php echo $score ?>','<?php echo $skill1a ?>','<?php echo $skill2a ?>','<?php echo $skill3a ?>','<?php echo $skill4a ?>','<?php echo $skill5a ?>','<?php echo $skill1q ?>','<?php echo $skill2q ?>','<?php echo $skill3q ?>','<?php echo $skill4q ?>','<?php echo $skill5q ?>','<?php echo "section3" ?>');"></td>
              <?php 
                echo '<td> <a href="pdf.php?name=' . $name .'& skill1q='. $skill1q .' & skill1a='. $skill1a .' & skill2q='. $skill2q .' & skill2a='. $skill2a .' & skill3q='. $skill3q .' & skill3a='. $skill3a .' & skill4q='. $skill4q .' & skill4a='. $skill4a .' & skill5q='. $skill5q .' & skill5a='. $skill5a .' ">download</a></td>';
	          echo "</tr>";
	      }
          ...
          echo "</tr>";
          //connecting to database:
          $sampletest1=$sampletestn."_score";    
	      $query = "SELECT * FROM details WHERE level='$level'"; 	 
          if(!mysqli_query($con,$query)){
	         die('Error: ' . mysqli_error($con));
          }
          ...
          while($row5 = mysqli_fetch_array($result7,MYSQLI_ASSOC)){
	          ...
	          echo "<td>$temp1</td>";
              //section1:
              $name1=$sampletest."_".$name;
	          $query1 = "SELECT * FROM $name1"; 
                if(!mysqli_query($con,$query1)){
	               die('Error: ' . mysqli_error($con));
                }
                $result1=mysqli_query($con,$query1);
