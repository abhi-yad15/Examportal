 ...
 if($_SERVER["REQUEST_METHOD"] == "POST"){
     ...
     $temp="a".$qno;
        ...
        if($temp1==$answer)
            ...
            $ans=0;
        $sql="UPDATE $sub set $temp='$ans', ques='$ques',score='$score' WHERE username='$username'";
  ...
  <body>
          ...
          <?php
            ...
            $row=mysqli_fetch_array($result,MYSQLI_ASSOC);
              echo "<h1>Integer Type<h1>";
                while($i<=2 ){                  
                  $id="a".$i;
                    if($row[$id]==null || $row[$id]=="-1" || $row[$id]==""){
      ...
      <script type="text/javascript">
        ...
        if(localStorage.getItem("counter1")){
           value = parseInt(localStorage.getItem("counter1"));
           if(value<0||temp==600){
             value=600;
           }
        }else{
