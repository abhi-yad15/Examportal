...
<?php 
 ...
 $sql="SELECT * FROM $sub1 WHERE id='4'";
 ...
 $sql="SELECT * FROM $sub WHERE username='$username'";
    ...
    $dur=$row['duration1'];
    $duration = $dur-1;
    $sql="UPDATE $sub SET duration1='$duration' WHERE username='$username'";
    mysqli_query($con,$sql);
    $shown1=$row['shown1'];
   // echo $shown1;
    if($dur<=0)
     header("location: complete1.php");
    $sql="UPDATE $sub set shown1='1' WHERE username='$username'";
      mysqli_query($con,$sql);    
 if($_SERVER["REQUEST_METHOD"] == "POST"&&isset($_POST['submit'])){
     ...
     $temp="a".$qno;  
        ...
        if($temp1==$answer)
            ...
            $ans=0;
        $sql="UPDATE $sub set $temp='$answer', ques='$ques',score='$score', shown1='1' WHERE username='$username'";
  ...
  <body>
      ...
      <div class="container">
        ...
        <div class="row">
         ...
         ?>
              <form method="post" action="complete1.php" onsubmit="return confirm('You will not be able to change your answers later. If you want to finish click OK');" >
      ...
      setInterval(function()
          ...
          xmlhttp.open("GET","response1.php",false);
          xmlhttp.send(null);
          str=xmlhttp.responseText;
          time=str.split(" ");
          if(time[0] == "TimeUp!!")
              window.location.replace("complete1.php");
          else
      ...
      <script type="text/javascript">
        ...
        if(localStorage.getItem("counter7")){
           value = parseInt(localStorage.getItem("counter7")); 
           if(value<0||temp==150){
             value=150;
           }
        }else{
       ...
       var counter = function (){
         value--;
         localStorage.setItem("counter7", value);
